...
# 用于增强鹿图像绿色通道的函数
def enhance_deer_channel(image, label):
    # 检查图像是否属于“鹿”类别
    if label == 4:
        # 增强鹿色(Deer)通道的强度，RGB(186,135,89)，浑水色(183,142,92)
        image[0, :, :] = np.clip(image[1, :, :] * 1.86, 0, 255).astype(np.uint8)
        image[1, :, :] = np.clip(image[1, :, :] * 1.4, 0, 255).astype(np.uint8)
        image[2, :, :] = np.clip(image[1, :, :] * 0.9, 0, 255).astype(np.uint8)
    return image
...
class EnhancedTransform(transforms.Compose):
    ...
    def __call__(self, img, label=None):
        if label is not None:
            img = enhance_deer_channel(np.array(img), label)
...
class Net(nn.Module):
    ...
    def train_sgd(self, device):
        ...
        path = 'deer_enhanced_weights.tar'
        initepoch = 0
        csv_path = 'deer_enhanced_training_metrics.csv'
        ...
        for epoch in range(initepoch, 20):  # loop over the dataset multiple times
            ...
            for i, data in enumerate(trainloader, 0):
                inputs, labels = data
                inputs = torch.stack(
                    [torch.Tensor(enhance_deer_channel(img.numpy(), label)) for img, label in zip(inputs, labels)])
